...
//ub6BUOchwHm1XBDG   gff0BflZxrh08g62
const dbURL ="mongodb+srv://shalinie:gff0BflZxrh08g62@cluster0.kyn9d.mongodb.net/sharecords?retryWrites=true&w=majority";
const app = express();
...
app.post("/register", (req, res) => {
  mongoClient.connect(dbURL, (err, client) => {
    if (err) throw err;
    let db = client.db("sharecords");
    db.collection("studentdetails").findOne({ email: req.body.email }, (err, data) => {
      if (err) throw err;
      if (data) {
        ...
        bcrypt.genSalt(10, (err, salt) => {
          bcrypt.hash(req.body.password, salt, (err, cryptPassword) => {
            ...
            db.collection("studentdetails").insertOne(req.body, (err, result) => {
              if (err) throw err;
              client.close();
              res
                .status(200)
